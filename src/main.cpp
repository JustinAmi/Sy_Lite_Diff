...
#if defined(NDEBUG)
# error "Sycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x74302f9513cc75ed687954aebde47d815293a2b870c539c5fa97f7272a3fecb9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sycoin Signed Message:\n";
...
{
    // Sycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000000); // Sycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; //10 * 30; // Sycoin: 3.5 days
static const int64 nTargetSpacing = 10 * 60;//1 * 30; // Sycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x74302f9513cc75ed687954aebde47d815293a2b870c539c5fa97f7272a3fecb9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Lion Air crash: No answers as to cause of jet disaster as bodies pulled from sea";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1540887335;
        ...
        assert(block.hashMerkleRoot == uint256("0x324110080bec59ae04ab26eec5eb5f83ad0b1c67c696fd0094aa7521421c4816"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xd3, 0xd4, 0xb1 }; // Sycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SycoinMiner(CWallet *pwallet)
{
    printf("SycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sycoin-miner");
    ...
    try { loop {
        ...
        printf("Running SycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SycoinMiner, pwallet));
}
